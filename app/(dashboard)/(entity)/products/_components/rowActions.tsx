...
export function DataTableRowActions<TData>({row}: DataTableRowActionsProps<TData>) {
    ...
    const handleDeleteProduct = async (id: string) => {
        try{
            ...
            toast.error("Something went wrong(Most likely product still have products in store!)");
        }
    }
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-24 bg-white">
        <DropdownMenuItem>
          <Button variant="ghost"><Link href={`/products/${product.id}`} className="flex flex-row items-center"><Pen className="h-3 w-3 text-slate-600 mr-2"/>Edit  </Link>
          </Button>
          </DropdownMenuItem>
        <DropdownMenuItem>
