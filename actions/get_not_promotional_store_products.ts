...
export const getNotPromotionalStoreProducts = async () => {
  try {
  ...
  WHERE promotional_product=false;`;
    ...
    console.error('Error fetching not promotional store products:', error);
    throw new Error('Failed to fetch not promotional store products');
  }
};